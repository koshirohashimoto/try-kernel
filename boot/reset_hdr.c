...
static void init_peri(void)
{
    ...
    /* Config for GPIO */
    out_w(GPIO_OE_CLR, (1<<25));    // Invalidate P25 terminal output
    out_w(GPIO_OUT_CLR, (1<<25));   // Clear P25 terminal output
    out_w(GPIO_CTRL(25), 5);        // P25 terminal SIO
    out_w(GPIO_OE_SET, (1<<25));    // Validate P25 terminal output

    out_w(GPIO_CTRL(0), 2);         // P0 terminal UART0-TX
    out_w(GPIO_CTRL(1), 2);         // P1 terminal UART0-RX
...
static void init_section(void)
{
    ...
    /* Initialize data section (Copy from rodata to data) */
    src = (_UW*)&__data_org;;
    ...
    /* Initialize bss section （0 clear） */
...
void Reset_Handler(void)
{
    ...
    /* Set the priority for PendSVC exception and SysTick exception */
